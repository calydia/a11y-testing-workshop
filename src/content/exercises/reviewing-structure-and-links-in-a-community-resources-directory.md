...
expectedFindings: 5
solution:
  summary: The directory contains five pattern-level findings. Its introductory heading sequence, descriptive introductory links, Digital skills support card, and closing section provide valid comparisons.
  findings:
...
1. Review the page visually and predict how its sections and resource cards should appear in a heading outline.
2. Navigate through every heading in document order, then inspect your screen reader's headings list or rotor.
3. Compare what your screen reader exposes with the visible organization.
4. Navigate through links sequentially, then inspect the links list or rotor without relying on surrounding text.
5. Identify exactly five accessibility findings while noting examples that work correctly.
6. For each finding, record the affected content, observed output, expected result, user impact, and remediation direction.
