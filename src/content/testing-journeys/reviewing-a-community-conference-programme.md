...
description: Conduct an intermediate review of programme structure, schedule-table relationships, common controls, graphical content, language changes, and session-detail dialogs.
status: published
order: 20
topics: [screen readers, headings, links, data tables, headers, controls, states, images, SVG, language, modal dialogs, reporting]
prerequisites:
  - Basic screen-reader operation
difficulty: intermediate
estimatedMinutes: 105
scenario: A community conference team is preparing to publish its programme. You have been asked to review whether screen-reader users can understand the programme and its schedule table, choose sessions using its content and controls, follow multilingual information, inspect session details, and return without losing their place.
...
objectives:
  ...
  - Navigate the programme schedule and assess its table name and row-and-column relationships.
...
stages:
  ...
  - title: Find your way through the programme
    task: Locate the keynote and selected sessions using heading navigation, sequential reading, link lists, and the at-a-glance schedule. Record the schedule name and navigate representative cells to assess whether its time, day, and room relationships are exposed. Check whether programme structure and session destinations remain understandable outside the visual layout.
    methods: [screen-reader-page-structure-and-links, testing-data-tables-with-a-screen-reader]
  ...
  - title: Consolidate and recommend
    ...
    methods: [screen-reader-page-structure-and-links, testing-data-tables-with-a-screen-reader, testing-controls-with-a-screen-reader, testing-image-alternative-text, screen-reader-icons-and-svg, screen-reader-language-changes, testing-modal-dialogs]
deliverables:
...
For every observation, record the programme state, navigation or interaction steps, actual result, expected result, and user impact. For the schedule, include its exposed name and representative row-and-column relationships. Keep direct observations separate from assumptions. Combine evidence when several techniques expose one underlying problem, and record useful behavior that passes.
